"""Build Parquet v1 dataset from event-level NetCDF files."""
...
def main() -> None:
    ...
    # Debug limiter: set cfg.max_files > 0 to process only the first N files.
    if cfg.max_files > 0:
        ...
        try:
            with open_netcdf_event(nc_path) as dataset:
                ...
                fo_features = compute_event_fo_features(
                    ...
                    frequency_bins_hz=cfg.frequency_bins_hz,
                    reductions=cfg.channel_reductions,
                    include_time_domain=cfg.feature_families.fo_time_domain,
                    include_spectral=cfg.feature_families.fo_spectral_bands,
                )
    ...
    # Skip records split by level for easier auditing
    skipped_event_records = []
    skipped_sensor_records = []
    for rec in skipped:
        payload = {
    ...
    print("=" * 80)
    print("PARQUET V1 BUILD COMPLETE")
    print("=" * 80)
    ...
    print(f"Missing z-axis count: {summary['missing_z_axis_count']}")
