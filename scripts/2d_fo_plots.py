...
def apply_bruno_psd_2d(
    ...
    """
    Compute PSD for each channel using Bruno's TimeSignalProcessing.psd().
    Returns:
        f_psd : (n_f,) frequency vector [Hz]
        PSD   : (n_f, n_ch) power spectral density (linear units from TSP)
    """
    x = np.asarray(data_2d, dtype=np.float64)
    n_t, n_ch = x.shape
    assert t.shape[0] == n_t, "t must match number of time samples"

    # First channel: get frequency grid and PSD length
    tsp0 = TimeSignalProcessing(time=t, signal=x[:, 0], Fs=fs,
                                window_size=window_size, window=window)
    tsp0.psd()
    f_psd = np.asarray(tsp0.frequency_Pxx)
    PSD = np.empty((f_psd.size, n_ch), dtype=np.float64)
    PSD[:, 0] = np.asarray(tsp0.Pxx)

    # Remaining channels
    for ch in range(1, n_ch):
        tsp = TimeSignalProcessing(time=t, signal=x[:, ch], Fs=fs,
                                   window_size=window_size, window=window)
        tsp.psd()
        PSD[:, ch] = np.asarray(tsp.Pxx)

    return f_psd, PSD


# ---- Paths ----
fo_h5_file = r"E:\recording_2024-08-29T08_01_16Z_5kHzping_1kHzlog_1mCS_10mGL_3000channels\sensor_2024-08-29T080549Z.h5"
out_dir = Path(r"D:\fo_test")
out_dir.mkdir(parents=True, exist_ok=True)
stem = Path(fo_h5_file).stem  # This takes the name -> 'sensor_2024-08-29T080549Z'

with h5py.File(fo_h5_file, 'r') as file:
    raw_fo = file['Acquisition']['Raw[0]']['RawData']  # (n_time, n_channels)
    raw_fo = raw_fo[:]  # load into memory
    n_t, n_ch = raw_fo.shape

    # Metadata for operations
    metadata = {
        "fs": float(file['Acquisition']['Raw[0]'].attrs['OutputDataRate']),  # Hz
        "dx": float(file['Acquisition'].attrs['SpatialSamplingInterval']),  # meters
        "gauge_length": float(file['Acquisition'].attrs['GaugeLength']),
        "fibre_refractive_index": float(file['Acquisition']['Custom'].attrs['Fibre Refractive Index']),
    }
...
strain_fo_data = from_opticalphase_to_strain(demeaned_fo_data, metadata)
...
# 2. Apply the bandpass filter (per channel) to the demeaned data
