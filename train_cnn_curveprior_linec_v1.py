...
def evaluate(
    ...
    return metrics, preds_log, targets_log, c_hat_all, c_target_all
...
def main():
    ...
    train_metrics, train_preds_log, train_targets_log, train_c_hat, train_c_target = evaluate(
        model, train_loader, device, cfg, n_track1, n_track2, cfg.features.r0
    )
    val_metrics, val_preds_log, val_targets_log, val_c_hat, val_c_target = evaluate(
        model, val_loader, device, cfg, n_track1, n_track2, cfg.features.r0
    )
    test_metrics, test_preds_log, test_targets_log, test_c_hat, test_c_target = evaluate(
