"""Build the multi-channel FO waveform dataset (waveform v2).

Stores a local channel window (1184-1204 = 21 channels) per event as a
(n_channels, T) tensor.  Output array shape: (N_events, 21, T).

Output  (holten_waveform_v002_<timestamp>/)
-------
* waveforms.npy        — float32 (N_events, 21, T)
* event_index.parquet  — one row per event: event_id, waveform_row_idx,
                         metadata, crop diagnostics
* build_config.json, build_log.txt
* plots/crop_diag_*.png — channel x time image + crop window per sample event

One block per event; the CNN Dataset maps each sensor target row to its event
block by index.  Same canonical 1,697-event set as v1.

Run:
    python build_waveform_v2.py
"""
...
from src.db.waveform.config_waveform_v2 import CONFIG
from src.db.waveform.waveform_v2_utils import extract_channel_window, process_event_block
...
def _plot_crop_diag(event_id, nc_path, cfg, label, out_path):
    ...
    with nc.Dataset(nc_path, "r") as ds:
        proc, _ = process_event_block(
            ...
            expected_n_channels=scfg.n_channels,
        )
    if proc is None:
        return
    ...
    # Top: full processed channel x time image with crop span.
    ...
    if proc.was_cropped:
        axes[0].axvspan(proc.crop_start_processed / fs, proc.crop_end_processed / fs,
    ...
    axes[0].axhline(scfg.center_channel, color="k", lw=0.8, ls=":")
    ...
    axes[0].set_title(
        ...
        f"|  shape={proc.block.shape}"
    )

    # Bottom: centre-channel trace with crop.
    ...
    axes[1].set_ylabel(f"strain ch {scfg.center_channel}")
...
def _select_events(index_df):
    ...
    sel["p95_long"] = ok.loc[(dur - dur.quantile(0.95)).abs().idxmin(), "event_id"]
...
def main() -> None:
    ...
    log("=" * 70)
    log("Build Waveform v2 — multi-channel FO time-series dataset")
    log("=" * 70)
    log(f"Channels     : {scfg.channel_lo}-{scfg.channel_hi}  ({n_ch} channels, "
        f"centre {scfg.center_channel})")
    ...
    # 3. Process
    log("\n[3/5] Processing multi-channel blocks ...")
    ...
    if coverage < 0.99:
        ...
        log(f"  WARNING: only {n_ok}/{len(canonical_ids)} events ({coverage*100:.1f}%). "
            "Re-run if network-drive related.")
        log("  " + "!" * 60)
    ...
    build_config = {
        ...
        "target_fs_hz": scfg.target_fs_hz, "native_fs_hz": scfg.native_fs_hz,
        "center_channel": scfg.center_channel,
        "channel_lo": scfg.channel_lo, "channel_hi": scfg.channel_hi,
        "n_channels": n_ch,
