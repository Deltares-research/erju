...
This module is not part of the current production database pipeline.
Current pipeline entrypoint is main_database_creation.py using src/db/netcdf_creator.py.
Kept for historical reference and potential method reuse.
"""
...
from src.utils.plot_FO_data import plot_signals_and_stalta
...
class CreateDatabase:
    ...
    def __init__(
        self,
        fo_data_path: str,
        acc_data_path: str = None,
        logbook_path: str = None,
    ...
    def from_accel_get_windows(
        ...
        accel_windows = AccelDataTimeWindows(
            accel_data_path=self.acc_data_path,
        ...
        file_names = get_files_in_dir(
            folder_path=self.acc_data_path, file_format=".asc", keep_extension=False
        )
        ...
        accel_data_df = accel_windows.extract_accel_data_from_file(
            file_name=file_names[0], no_cols=3
        )
        ...
        self.window_indices, self.window_times = (
            accel_windows.detect_accel_events_sta_lta(
                accel_data=accel_data_df,
                nsta=nsta,
                nlta=nlta,
    ...
    def from_windows_get_fo_signal(self):
        ...
        file_names = get_files_in_dir(
            folder_path=self.fo_data_path, file_format=".tdms"
        )
    ...
    def extract_accel_windows(self, file_name: str, nsta: int = 1, nlta: int = 8):
        ...
        accel_windows = AccelDataTimeWindows(
            accel_data_path=self.acc_data_path, logbook_path=self.logbook_path
        )

        # Extract the accelerometer data from the file with default number of columns (3)
        accel_data_per_file = accel_windows.extract_accel_data_from_file(
            file_name=file_name
        )
        ...
        accel_windows_indices, accel_windows_times = (
            accel_windows.detect_accel_events_sta_lta(
        ...
        accel_windows_indices, accel_windows_times = (
            accel_windows.filter_windows_with_logbook(
                time_buffer=15,
    ...
    def find_matching_fo_files(
        self,
        accel_windows_times: list,
        buffer_seconds: int = 35,
        file_time_coverage: int = 30,
    ):
        """
        ...
        fo_file_names = get_files_in_dir(
            folder_path=self.fo_data_path, file_format=".tdms"
        )
        ...
        for window_time in accel_windows_times:
            ...
            if matching_files:
                ...
                sorted_files = sorted(
                    matching_files,
                    key=lambda name: extract_timestamp_from_name([name])[0],
                )

                # Get the start and end times of the fo files
                file_times = [
                    (
                        extract_timestamp_from_name([file])[0],
                        extract_timestamp_from_name([file])[0]
                        + timedelta(seconds=file_time_coverage),
                    )
    ...
    def extract_and_join_fo_data(self, fo_file_names: str, channel_no: int = 4270):
        ...
        file_instance = BaseFOdata.create_instance(
        ...
        for file in file_names:
            ...
            time = [
                initial_time + timedelta(seconds=i * delta_time)
    ...
    def create_pickle_database(self, channel_no: int = 4270, save_path: str = None):
        ...
        accel_file_names = get_files_in_dir(
            folder_path=self.acc_data_path, file_format=".asc", keep_extension=False
        )
        ...
        for accel_file in accel_file_names:
            ...
            print(
                f"Processing file: {accel_file} ...................................................................."
            )

            # Get accelerometer time windows and indices for a specific file
            accel_window_indices, accel_window_times, accel_data_per_file = (
                self.extract_accel_windows(file_name=accel_file)
            )
            ...
            for i, accel_window in enumerate(accel_window_times):
                ...
                if not fo_file_names_per_window[i]:
                    print(
                        f"No FO data available for window {i + 1}/{len(accel_window_times)}"
                    )
                    continue

                # Join the selected list of FO files into a single signal dataframe
                fo_data_df = self.extract_and_join_fo_data(
                    fo_file_names=fo_file_names_per_window[i], channel_no=channel_no
                )
                ...
                fo_data_in_window = fo_data_df[
                    (fo_data_df["time"] >= start_time)
                    & (fo_data_df["time"] <= end_time)
                ].copy()

                # Trim the accelerometer data to match exactly the FO signal data
                accel_data_in_window = accel_data_per_file[
                    (accel_data_per_file["T(ms)"] >= start_time)
                    & (accel_data_per_file["T(ms)"] <= end_time)
                ].copy()
                ...
                safe_start_time = start_time.strftime(
                    "%Y%m%d_%H%M%S%f"
                ...
                print(
                    f"Saved pickle file: {pickle_file_name}, for window {i + 1}/{len(accel_window_times)}"
                )
    ...
    def create_netcdf_file(self, output_folder, data_dict):
        ...
        """
        # Sanitize the start_time to create a valid file name
        safe_start_time = data_dict["start_time"].strftime(
            "%Y%m%d_%H%M%S%f"
        )  # Format to safe filename
        ...
        with nc.Dataset(netcdf_file_path, "w", format="NETCDF4") as dataset:
            ...
            timeseries_var = dataset.createVariable(
                "signal", np.float32, ("time",), zlib=True, complevel=9
            )
    ...
    def extract_all_events(self, selected_channel: int = 4270, threshold: int = 500):
        ...
        signal_mean = file_instance.signal_averaging(
            file_type=".tdms",
        ...
        files_with_trains = file_instance.get_files_above_threshold(
            file_type=".tdms", signal=signal_mean, threshold=threshold
        )
        print("Selected files: ", files_with_trains)

        # Save the name of the files with trains in a txt
        file_instance.save_txt_with_file_names(
            save_to_path=self.output_path,
            selected_files=files_with_trains,
        ...
        for file in files_with_trains:
            ...
            extended_signal = self.extract_and_join_fo_data(
                fo_file_names=[file_before, file, file_after],
                channel_no=selected_channel,
            )

            # Find the events with sta/lta method from the extended signal data
            window_indices, window_times, stalta_ratio = (
                file_instance.detect_FO_events_sta_lta(
            ...
            for i, window_time in enumerate(window_times):
                ...
                # Extract from the FO signal data only the data that is within the time window
                fo_data_in_window = extended_signal[
                    (extended_signal["time"] >= start_time)
                    & (extended_signal["time"] <= end_time)
                ].copy()
                ...
                self.create_netcdf_file(
                    output_folder=self.output_path, data_dict=data_dict
                )
...
database = CreateDatabase(fo_data_path=fo_data_path, output_path=path_save_database)
