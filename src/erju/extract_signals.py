...
Kept for historical analysis workflows and potential utility reuse.
"""
...
def data_consistency_check(file_path_list: list) -> bool:
    ...
    if first_file_sf != last_file_sf:
        raise ValueError(
            f"Sampling frequency mismatch: {first_file_sf} Hz (first file) vs. {last_file_sf} Hz (last file)"
        )
    elif first_file_data_shape != last_file_data_shape:
        raise ValueError(
            f"Data shape mismatch: {first_file_data_shape} (first file) vs. {last_file_data_shape} (last file)"
        )
    else:
...
def do_stalta(
    ...
    cft = recursive_sta_lta(
        a=trace.data, nsta=int(lower * freq), nlta=int(upper * freq)
    )
...
def find_trains_STALTA(
    ...
    values = do_stalta(
        data=data,
        freq=sf
        / 2,  # It works better at finding events with sf/2 instead of the actual sf that I initially used
...
def create_netcdf_file(
    data, timestamps, start_time, end_time, sampling_frequency, output_dir="output"
):
    """
    ...
    with nc.Dataset(file_path, "w", format="NETCDF4") as ncfile:
        ...
        # Assign data
        time_var[:] = np.array(
            [(ts - start_time).total_seconds() for ts in event_timestamps]
        )
...
output_dir = (
    r"N:\Projects\11210000\11210064\B. Measurements and calculations\holten\fo_plot"
)
...
start_channel = max(
    0, center_channel - channel_range
)  # Ensure start channel is not negative
...
for batch_number, batch in enumerate(file_batches):
    ...
    for file_path in batch:
        ...
        with h5py.File(file_path, "r") as file:
            ...
            file_raw_data = file["Acquisition"]["Raw[0]"]["RawData"][
    ...
    for channel in range(raw_data.shape[1]):
        ...
        raw_data_bandpass[:, channel] = bandpass(
            data=raw_data_highpass[:, channel],
            freqmin=0.1,
            freqmax=100,
            fs=1000,
            corners=4,
    ...
    try:
        ...
        window_indices, window_times, values = find_trains_STALTA(
            data=raw_data_highpass[:, relative_center_channel],
            timestamps=timestamps,
        ...
        if len(window_indices) > 0:
            logger.info(
                f"Detected {len(window_indices)} events in batch {batch_number + 1}"
            )

        # Plot the signal and STA/LTA ratio with detected events using the STA/LTA method
        plot_signals_and_stalta(
            raw_signal=raw_data[:, relative_center_channel],
