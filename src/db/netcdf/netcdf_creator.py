    ...
    for local_idx, (event_id, event_data) in enumerate(events_dict.items(), start=1):
        ...
        with nc.Dataset(netcdf_path, "w", format="NETCDF4") as dataset:
            ...
            if fo_data and fo_data.get("found", False):
                ...
                fo_first_channel = fo_data.get("fo_first_channel")
                if fo_first_channel is not None:
                    first_ch_var = geometry_fo_grp.createVariable(
                        "fo_first_channel", "i4"
                    )
                    first_ch_var[:] = np.int32(fo_first_channel)
                    first_ch_var.long_name = (
                        "First absolute FO channel index selected from the DAS array"
                    )

                fo_last_channel = fo_data.get("fo_last_channel")
                if fo_last_channel is not None:
                    last_ch_var = geometry_fo_grp.createVariable(
                        "fo_last_channel", "i4"
                    )
                    last_ch_var[:] = np.int32(fo_last_channel)
                    last_ch_var.long_name = (
                        "Last absolute FO channel index selected from the DAS array"
                    )
