...
def _data_root() -> Path:
    return (
        Path(r"P:\11210978-erju-ai") if os.name == "nt" else Path("/p/11210978-erju-ai")
    )
...
class ArchConfig:
    ...
    # Pooling: legacy mean pooling or amplitude-preserving mean/std/max pooling.
    pooling: str = "mean"  # "mean" | "stats"
    # S6: infer valid (non-zero-padded) samples directly from the waveform.
    auto_valid_mask: bool = False
    padding_zero_tol: float = 0.0
    # S6: concatenate raw FO amplitude summaries before the prediction head.
    use_raw_amp_features: bool = False
    raw_amp_local_half_width: int = 5
    raw_amp_include_crest: bool = True
...
class TrainHyperparams:
    ...
    # Auxiliary loss on the event-level common spectral amplitude.
    # Implemented as MSE(mean(predicted standardized bands), mean(target bands)).
    spectral_amp_weight: float = 0.0
    # Validation/inference are forced to float32 for numerical stability.
    eval_float32: bool = True
    # Fail fast on obviously unstable standardized outputs.
    max_abs_standardized_output: float = 25.0
...
def get_config(name: str, seed: int = 42) -> SpectralExperimentConfig:
    ...
    # ── S6: amplitude-preserving ResNet successor to S3 ──────────────────────
    elif name == "S6_amp":
        return SpectralExperimentConfig(
            name="S6_amp",
            description=(
                "S3 residual formulation with no BatchNorm, automatic padding "
                "mask, mean/std/max statistics pooling, and raw FO amplitude features"
            ),
            seed=seed,
            target_type="spectral",
            use_meta_residual=True,
            arch=ArchConfig(
                ...
                activation="silu",
                pooling="stats",
                auto_valid_mask=True,
                padding_zero_tol=0.0,
                use_raw_amp_features=True,
                raw_amp_local_half_width=5,
                raw_amp_include_crest=True,
                meta_hidden=[32, 32],
                meta_dropout=0.05,
                head_hidden=[128, 64],
                head_dropout=0.15,
            ...
            train=TrainHyperparams(
                ...
                scheduler="plateau",
                mixed_prec=True,
                spectral_amp_weight=0.30,
                eval_float32=True,
                max_abs_standardized_output=25.0,
            ),
        )
    ...
    # ── P1: Matched direct-PGV control ───────────────────────────────────────
        ...
        raise ValueError(
            f"Unknown config name: '{name}'. " f"Valid: S1, S2, S3, S4, S5, S6_amp, P1"
        )


ALL_CONFIG_NAMES = ["S1", "S2", "S3", "S4", "S5", "S6_amp", "P1"]
SPECTRAL_CONFIG_NAMES = ["S1", "S2", "S3", "S4", "S5", "S6_amp"]
PGV_CONFIG_NAMES = ["P1"]
